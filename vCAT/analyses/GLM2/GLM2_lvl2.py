...
=============================================
Second level workflow for Seimens vCAT task data

- vCAT GLM2
  - Use FSL ROI to recreate EPI data
  - EV directory (GLM2) --- /home/data/madlab/Mattfeld_vCAT/behav/sub-0??/GLM2

- python GLM2_lvl2.py  -s sub-0??
                       -o /home/data/madlab/Mattfeld_vCAT/derivatives/GLM2/lvl2/sub-0??
                       -w /scratch/madlab/crash/mandy/vcat/GLM2/lvl2/sub-0??
"""
...
def secondlevel_wf(subject_id, 
    ...
    all_contrasts = ['AllVsBase', 'fixed_corr', 'fixed_incorr', 'cond_corr', 'cond_incorr', 'remaining',
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir,
                          'GLM2/lvl1/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir,
		             'GLM2/lvl1/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'GLM2/lvl1/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'GLM2/lvl1/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'GLM2/lvl1/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/vcat/GLM2/lvl2'
