...
"""
==============================================
MVPA_fMRI: Functional Localizer  -- Model MVPA
==============================================
First level workflow for Seimens MVPA functional localizer data

- vCAT MVPA
  - Use FSL ROI to recreate EPI data
  - EV directory (MVPA) --- /home/data/madlab/Mattfeld_vCAT/behav/vCAT_0??/MVPA

- python MVPA_lvl1.py  -s vCAT_0??
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,3):
        names = []
        onsets = []
        durations = []
        amplitudes = []

        faces = np.genfromtxt(base_dir + '{0}/MVPA/run{1}_faces.txt'.format(subject_id, curr_run), dtype = str)
        scenes = np.genfromtxt(base_dir + '{0}/MVPA/run{1}_scenes.txt'.format(subject_id, curr_run), dtype = str)
        maths = np.genfromtxt(base_dir + '{0}/MVPA/run{1}_maths.txt'.format(subject_id, curr_run), dtype = str)        

        sequence = ['faces', 'scenes', 'maths']
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        if 'faces' in j.conditions and 'scenes' in j.conditions and 'maths' in j.conditions:
            cont_faces_vs_scenes = ['faces_minus_scenes', 'T', ['faces','scenes'], [1, -1]]
            cont_scenes_vs_faces = ['scenes_minus_faces', 'T', ['faces','scenes'], [-1, 1]]
            cont_faces_vs_maths = ['faces_minus_maths', 'T', ['faces','maths'], [1, -1]]
            cont_maths_vs_faces = ['maths_minus_faces', 'T', ['faces','maths'], [-1, 1]]
            cont_scenes_vs_maths = ['scenes_minus_maths', 'T', ['scenes','maths'], [1, -1]]
            cont_maths_vs_scenes = ['maths_minus_scenes', 'T', ['scenes','maths'], [-1, 1]]

            curr_run_contrasts.append(cont_faces_vs_scenes)
            curr_run_contrasts.append(cont_scenes_vs_faces)
            curr_run_contrasts.append(cont_faces_vs_maths)
            curr_run_contrasts.append(cont_maths_vs_faces)
            curr_run_contrasts.append(cont_scenes_vs_maths)
            curr_run_contrasts.append(cont_maths_vs_scenes)
...
def firstlevel_wf(subject_id, sink_directory, name = 'vcat_frstlvl_wf'):
    ...
    datasource.inputs.base_directory = os.path.abspath('/home/data/madlab/Mattfeld_vCAT/derivatives/preproc_loc/')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/vcat/MVPA/lvl1'
