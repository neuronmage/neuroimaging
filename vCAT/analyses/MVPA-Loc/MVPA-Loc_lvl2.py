...
=============================================
Second level workflow for Siemens vCAT functional localizer data.

- vCAT MVPA
  - Use FSL ROI to recreate EPI data
  - EV directory (MVPA) --- /home/data/madlab/Mattfeld_vCAT/behav/sub-0??/MVPA

- python MVPA_lvl2.py  -s vCAT_0??
                       -o /home/data/madlab/Mattfeld_vCAT/derivatives/MVPA/lvl2/sub-0??
                       -w /scratch/madlab/crash/mandy/vcat/MVPA/lvl2/sub-0??
"""
...
def secondlevel_wf(subject_id, 
                   sink_directory, 
                   name = 'MVPA_scndlvl_wf'):   
    scndlvl_wf = Workflow(name = 'scndlvl_wf')   
    base_dir = os.path.abspath('/home/data/madlab/Mattfeld_vCAT/derivatives/')
    
    all_contrasts = ['AllVsBase', 'faces', 'scenes', 'maths', 'faces_minus_scenes', 'scenes_minus_faces', 
                     'faces_minus_maths', 'maths_minus_faces', 'scenes_minus_maths', 'maths_minus_scenes']
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir,
                          'MVPA/lvl1/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir,
		             'MVPA/lvl1/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'MVPA/lvl1/%s/modelfit/contrasts/_estimate_model*/cope??_%s.nii.gz',
                                            varcopes = 'MVPA/lvl1/%s/modelfit/contrasts/_estimate_model*/varcope??_%s.nii.gz',
                                            mask_file = 'preproc_loc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'MVPA/lvl1/%s/modelfit/dofs/_estimate_model%s/%s')
...
def create_scndlvl_workflow(args, name = 'MVPA_scndlvl'):
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/vcat/MVPA/lvl2'
