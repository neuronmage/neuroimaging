...
"""
============================================
WMAZE_fMRI: Conditional Trials -- Model RSA1
============================================
...
- Model RSA1
  ...
  - EV directory (Model RSA1) --- /home/data/madlab/Mattfeld_WMAZE/sourcedata/behav/WMAZE_001/model_RSA1
- python RSA1_cond_lvl1.py -s WMAZE_001
                       	   -o /home/data/madlab/Mattfeld_WMAZE/dset/analyses/model_RSA1/lvl1/cond
                      	   -w /scratch/data/crash/mandy/learning/RSA1/cond/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_B_corr = np.genfromtxt(base_proj_dir + '/{0}/model_RSA1/run{1}_B_corr.txt'.format(subject_id,curr_run),dtype = str) 
        data_B_incorr = np.genfromtxt(base_proj_dir + '/{0}/model_RSA1/run{1}_B_incorr.txt'.format(subject_id,curr_run),dtype = str)
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/model_RSA1/run{1}_B_all_remaining.txt'.format(subject_id,curr_run),dtype = str)      
        ...
        #insert contents of each run at index of curr_run (1-6) 
...
#obtain and create contrasts *flexibly* in case there are not enough incorrect trials
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        if 'B_corr' in j.conditions and 'B_incorr' in j.conditions:
            cont_all_corr_vs_all_incorr = ['corr_minus_incorr', 'T', ['B_corr','B_incorr'], [1, -1]]
            cont_all_incorr_vs_all_corr = ['incorr_minus_corr', 'T', ['B_corr','B_incorr'], [-1, 1]]
...
#extracting the motion parameters from the noise files
...
def firstlevel_wf(subject_id,sink_directory, name = 'wmaze_frstlvl_wf'):
    ...
    #subjectinfo function with the name, onset, duration, and amplitude info 
    ...
    #outputspec node using IdentityInterface() to receive info from estimate_model, merge_contrasts, z2pval, and generate_model
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/RSA1/cond/lvl1/'
