...
"""
=================================================
Learning Analysis -- HALVES -- Conditional Trials
=================================================
Second level workflow for UM GE 750 wmaze task data
...
- python HALVES_C_lvl2.py -s WMAZE_001
                       	  -o /home/data/madlab/Mattfeld_WMAZE/dset/analyses/model_HALVES/lvl2/cond
                      	  -w /scratch/data/crash/mandy/learning/HALVES/cond/lvl2
"""
...
def num_copes_range(files): #function to determine range of copes list
...
def doublelist(x): #function to turn item into list
...
def get_contrasts(data_inputs): #function to isolate contrast names
    import os
...
def get_subs(subject_id, cons): #function to name contrast folders
    subs = []
...
def get_dofvolumes(dof_files, cope_files): #function to grab the correct dof file **usually the source of lvl2 crashes** 
    import os
    import nibabel as nb
    import numpy as np
    filenames = []   
    for cope_file_num, cope_file in enumerate(cope_files): #iterate through list of subject copes       
        cope_file_list = cope_file.split('/') #split filename for current cope file        
        cope_name = cope_file_list[-1][7:-7] #grab name of contrast
        img = nb.load(cope_file) 
        out_data = np.zeros(img.get_shape()) #Numpy zeros array with same dimensions as cope nii.gz      
        if len(out_data.shape) == 4: #if cope_file is 4D           
            for i in range(out_data.shape[-1]): #for each value in last dimension               
                dof = np.loadtxt(dof_files[cope_file_num][i]) #find appropriate dof file given current cope             
                out_data[:, :, :, i] = dof #assignment of dof value to all x, y, z at the current T dimension             
        else: #if the cope_file is only 3D 
...
def secondlevel_wf(subject_id, sink_directory, name = 'wmaze_scndlvl_wf'):   
    ...
    base_dir = os.path.abspath('/home/data/madlab/Mattfeld_WMAZE/dset/analyses/')
    
    all_contrasts = ['AllVsBase', 'all_remaining', 'early_learning', 'late_learning', 'early_nonlearning', 'late_nonlearning',
                     'early_learn_minus_late_learn', 'late_learn_minus_early_learn', 'early_nonlearn_minus_late_nonlearn', 
                     'late_nonlearn_minus_early_nonlearn', 'early_learn_minus_early_nonlearn', 'early_nonlearn_minus_early_learn',
                     'late_learn_minus_late_nonlearn', 'late_nonlearn_minus_late_learn', 'all_early_minus_all_late', 'all_late_minus_all_early']
    contrasts = []
    dof_runs = []

    for curr_cont in all_contrasts:
        cont_files = glob(os.path.join(base_dir, 
                          'learning/HALVES/lvl1/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for i, curr_file_list in enumerate(cnt_file_list):
        ...
        for curr_file in curr_file_list:
            dof_runs[i].append(curr_file.split('/')[-2][-1]) #grabs estimate_model#
    ...
    datasource.inputs.base_directory = os.path.abspath('/home/data/madlab/Mattfeld_WMAZE/')
    datasource.inputs.field_template = dict(copes = 'dset/analyses/learning/HALVES/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'dset/analyses/learning/HALVES/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'sourcedata/mri/preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'dset/analyses/learning/HALVES/lvl1/cond/%s/modelfit/dofs/_estimate_model%s/%s')
    ...
    #MapNode to merge copes into a single matrix across subject runs
    ...
    #node to define contrasts from cope names
    ...
    #MapNode to create fixed effects analysis (within sub across runs)
    ...
    #MapNode to run the fixed effects analysis
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/HALVES/cond/lvl2'
