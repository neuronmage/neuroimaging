...
"""
================================================================
Learning Analysis -- HALVES_C -- Fixed Before Conditional Trials
================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Learning Analysis Version HALVES (Derivative & Early vs. Late)
  - Use FSL ROI to recreate EPI data, removing last 3 volumes
  - Removed last 3 trials before EV creation
  - EV directory (model_HALVES_F) --- /home/data/madlab/Mattfeld_WMAZE/sourcedata/behav/WMAZE_001/model_HALVES_F
- python HALVES_fixed_lvl1.py -s WMAZE_001
                       	     -o /home/data/madlab/Mattfeld_WMAZE/dset/analyses/model_HALVES/lvl1/fixed
                      	     -w /scratch/data/crash/mandy/learning/HALVES/fixed/lvl1
...
def subjectinfo(subject_id):
    ...
    data_early_learning = np.genfromtxt(base_proj_dir+'/{0}/model_HALVES/set{1}_early_learn.txt'.format(subject_id,curr_set),dtype=str)

    data_late_learning = np.genfromtxt(base_proj_dir+'/{0}/model_HALVES/set{1}_late_learn.txt'.format(subject_id,curr_set),dtype=str)

    data_early_nonlearning=np.genfromtxt(base_proj_dir+'/{0}/model_HALVES/set{1}_early_nonlearn.txt'.format(subject_id,curr_set),dtype=str)

    data_late_nonlearning = np.genfromtxt(base_proj_dir+'/{0}/model_HALVES/set{1}_late_nonlearn.txt'.format(subject_id,curr_set),dtype=str)
 
    data_all_remaining = np.genfromtxt(base_proj_dir+'/{0}/model_HALVES/set{1}_all_remaining.txt'.format(subject_id,curr_set),dtype=str)  

 
    sequence = ['early_learning', 'late_learning', 'early_nonlearning', 'late_nonlearning']
...
def get_contrasts(subject_id, info):
    ...
    if 'early_learning' in j.conditions and 'late_learning' in j.conditions:
        cont_early_learn_vs_late_learn = ['early_learn_minus_late_learn', 'T', ['early_learning','late_learning'], [1, -1]]
        cont_late_learn_vs_early_learn = ['late_learn_minus_early_learn', 'T', ['late_learning','early_learning'], [1, -1]]
        contrasts.append(cont_early_learn_vs_late_learn)
        contrasts.append(cont_late_learn_vs_early_learn)
    if 'early_nonlearning' in j.conditions and 'late_nonlearning' in j.conditions:
        cont_early_nonlearn_vs_late_nonlearn = ['early_nonlearn_minus_late_nonlearn', 'T', ['early_nonlearning','late_nonlearning'], [1, -1]]
        cont_late_nonlearn_vs_early_nonlearn = ['late_nonlearn_minus_early_nonlearn', 'T', ['late_nonlearning','early_nonlearning'], [1, -1]]
        contrasts.append(cont_early_nonlearn_vs_late_nonlearn)
        contrasts.append(cont_late_nonlearn_vs_early_nonlearn)
    if 'early_learning' in j.conditions and 'early_nonlearning' in j.conditions:
        cont_early_learn_vs_early_nonlearn = ['early_learn_minus_early_nonlearn', 'T', ['early_learning','early_nonlearning'], [1, -1]]
        cont_early_nonlearn_vs_early_learn = ['early_nonlearn_minus_early_learn', 'T', ['early_nonlearning','early_learning'], [1, -1]]
        contrasts.append(cont_early_learn_vs_early_nonlearn)
        contrasts.append(cont_early_nonlearn_vs_early_learn)
    if 'late_learning' in j.conditions and 'late_nonlearning' in j.conditions:
        cont_late_learn_vs_late_nonlearn = ['late_learn_minus_late_nonlearn', 'T', ['late_learning','late_nonlearning'], [1, -1]]
        cont_late_nonlearn_vs_late_learn = ['late_nonlearn_minus_late_learn', 'T', ['late_nonlearning','late_learning'], [1, -1]]
        contrasts.append(cont_late_learn_vs_late_nonlearn)
        contrasts.append(cont_late_nonlearn_vs_late_learn)
    if 'early_learning' in j.conditions and 'early_nonlearning' in j.conditions and 'late_learning' in j.conditions and 'late_nonlearning' in j.conditions:
	cont_all_early_vs_all_late = ['all_early_minus_all_late', 'T', 
                                      ['early_learning','early_nonlearning','late_learning','late_nonlearning'], [1./2, 1./2, -1./2, -1./2]]
        cont_all_late_vs_all_early = ['all_late_minus_all_early', 'T', 
                                      ['late_learning','late_nonlearning','early_learning','early_nonlearning'], [1./2, 1./2, -1./2, -1./2]]
        contrasts.append(cont_all_early_vs_all_late)
        contrasts.append(cont_all_late_vs_all_early)
    return contrasts
...
def motion_noise(subjinfo, regressor_file):
    ...
    if subjinfo.regressor_names == None: #convert into arrays to allow appending
...
def firstlevel_wf(subject_id, sink_directory, name = 'HALVES_frstlvl_wf'):
...
def create_frstlvl_workflow(args, name = 'HALVES_frstlvl'):
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/HALVES_F/lvl1/1'
