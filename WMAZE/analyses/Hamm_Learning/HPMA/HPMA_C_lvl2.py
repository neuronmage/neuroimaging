...
"""
===============================================
Learning Analysis -- HPMA -- Conditional Trials
===============================================
...
- WMAZE Learning Analysis Version HPMA (Derivative & Early vs. Late & PMA)
  ...
  - EV directory (model_HPMA_C) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/model_HPMA_C
- python HPMA_C_fixed_lvl2.py -s WMAZE_001
                      	      -o /home/data/madlab/data/mri/wmaze/scndlvl/learning/HPMA/cond
                       	      -w /scratch/madlab/crash/mandy/learning/HPMA_C/lvl2
"""
...
def secondlevel_wf(subject_id, sink_directory, name = 'wmaze_scndlvl_wf'):    
    ...
    all_contrasts = ['AllVsBase', 'all_remaining', 'early_learn_event', 'late_learn_event', 'early_nonlearn_event', 'late_nonlearn_event',
                     'early_learn_pmod', 'late_learn_pmod', 'early_nonlearn_pmod', 'late_nonlearn_pmod',
                     'event_early_learn_minus_pmod_early_learn', 'pmod_early_learn_minus_event_early_learn', #early learn
                     'event_late_learn_minus_pmod_late_learn', 'pmod_late_learn_minus_event_late_learn', #late learn
                     'event_early_nonlearn_minus_pmod_early_nonlearn', 'pmod_early_nonlearn_minus_event_early_nonlearn', #early nonlearn
                     'event_late_nonlearn_minus_pmod_late_nonlearn', 'pmod_late_nonlearn_minus_event_late_nonlearn'] #late nonlearn
    contrasts = []
    dof_runs = []

    for curr_cont in all_contrasts:
        cont_files = glob(os.path.join(base_dir,'frstlvl/learning/HPMA/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, 
																	      curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir,'frstlvl/learning/HPMA/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, 
																		      curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/learning/HPMA/cond/%s/modelfit/contrasts/_estimate_model*/cope??_%s.nii.gz',
                                            varcopes = 'frstlvl/learning/HPMA/cond/%s/modelfit/contrasts/_estimate_model*/varcope??_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/learning/HPMA/cond/%s/modelfit/dofs/_estimate_model%s/%s')
    ...
    #MapNode to merge all copes into single matrix across subject runs
    ...
    #MapNode to merge all of the varcopes into a single matrix across subject runs per voxel
    ...
    #function node to define contrasts from the names of the copes
    ...
    #function node to rename output files to be more descriptive
    ...
    #MapNode to create the Fixed Effects analysis (aka within subj across runs)
    ...
    #MapNode to run FLAMEO for the fixed effects analysis
    flameo_fe = MapNode(FLAMEO(),  iterfield = ['cope_file', 'var_cope_file', 'dof_var_cope_file', 'design_file', 't_con_file', 'cov_split_file'],
                        name = 'flameo_fe')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/HPMA_C/lvl2'
