...
- WMAZE Learning Analysis Version HPMA_C (Derivative & Early vs. Late & PMA)
...
- python HAVLES_C_norm_stats.py -s WMAZE_001
                         -o /home/data/madlab/data/mri/wmaze/norm_stats/learning/HPMA/cond
                         -w /scratch/madlab/crash/mandy_learning/HPMA_C/norm_stats 
"""
...
work_dir = '/scratch/madlab/crash/mandy/learning/HPMA_C/norm_stats'
sink_dir = '/home/data/madlab/data/mri/wmaze/norm_stats/learning/HPMA/cond'
...
datasource_norm = Node(DataGrabber(infields = ['subject_id'], outfields = info_norm.keys()), name = "datasource_norm")
datasource_norm.inputs.base_directory = proj_dir
                                             
datasource_norm.inputs.field_template = dict(copes = 'scndlvl/learning/HPMA/cond/%s/fixedfx/cope*.nii.gz',
                                             varcopes = 'scndlvl/learning/HPMA/cond/%s/fixedfx/varcope*.nii.gz',
                                             bbreg_xfm = 'preproc/%s/bbreg/_fs_register0/%s*.mat', #BBReg transformation matrix
                                             ants_warp = 'norm_anat/%s/anat2targ_xfm/_subject_id_%s/%s*.h5', #ANTS transformation matrix
                                             mean_image = 'preproc/%s/ref/%s*.nii.gz') #mean reference image
...
#convert FreeSurfer-style Affine registration into ANTS compatible itk format (ITK = Insight Tool Kit -- medical processing library)
...
#concatenate ITK affine and ANTS transforms into a list
...
cope2targ = MapNode(ants.ApplyTransforms(), iterfield = ['input_image'], name = 'cope2targ')
...
cope2targ.inputs.reference_image = '/home/data/madlab/data/mri/wmaze/wmaze_T1_template/T_wmaze_template.nii.gz' #specify convertion space
...
varcope2targ = MapNode(ants.ApplyTransforms(), iterfield = ['input_image'], name = 'varcope2targ')
...
varcope2targ.plugin_args = {'sbatch_args': '-n 4 --tasks-per-node=4'}
...
#define contrasts from cope names
...
#rename output files with something more meaningful
