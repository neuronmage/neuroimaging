...
"""
=========================================================
WMAZE_fMRI: Fixed Before Conditional Trials -- Model RSA3
=========================================================
...
- Model RSA3
  ...
  - EV directory (Model RSA3) --- /home/data/madlab/Mattfeld_WMAZE/sourcedata/behav/WMAZE_001/model_RSA3
- python RSA3_lvl1.py -s WMAZE_001
                       	   -o /home/data/madlab/Mattfeld_WMAZE/dset/analyses/model_RSA3/lvl1/cond
                      	   -w /scratch/data/crash/mandy/learning/RSA3/cond/lvl1
...
def subjectinfo(subject_id): #function to gather contrast info into Bunch
    ...
    for curr_run in range(1,7): #iterate through runs
        ...
        data_learn = np.genfromtxt(base_proj_dir + '/{0}/model_RSA3/run{1}_learn.txt'.format(subject_id,curr_run),dtype = str) #load data
        data_nonlearn = np.genfromtxt(base_proj_dir + '/{0}/model_RSA3/run{1}_nonlearn.txt'.format(subject_id,curr_run),dtype = str)
        data_remaining = np.genfromtxt(base_proj_dir + '/{0}/model_RSA3/run{1}_remaining.txt'.format(subject_id,curr_run),dtype = str)            

        if data_nonlearn.size > 0: #more than one nonlearn trial
            curr_names = ['learn', 'nonlearn']
            curr_learn_onsets = map(float, data_learn[:,0]) #map funct converts all onset values from 1st column of EV file to float
            curr_learn_durations = map(float, data_learn[:,1]) #map funct converts all duration values from 2nd column
            curr_learn_amplitudes = map(float, data_learn[:,2]) #map funct converts all amplitude values from 3rd column
            if data_nonlearn.size == 3: #only one nonlearn trial 
                curr_nonlearn_onsets = [float(data_nonlearn[0])] #converts single trial onset value from 1st column of EV file to float
                curr_nonlearn_durations = [float(data_nonlearn[1])] #converts single trial duration value from 2nd column
                curr_nonlearn_amplitudes = [float(data_nonlearn[2])] #converts single trial amplitude value from 3rd column
            else: #more than one nonlearning trial
                curr_nonlearn_onsets = map(float, data_nonlearn[:,0]) 
                curr_nonlearn_durations = map(float, data_nonlearn[:,1])
                curr_nonlearn_amplitudes = map(float,data_nonlearn[:,2]) 
            ...
            curr_names = ['learn']
            curr_learn_onsets = map(float, data_learn[:,0])
            curr_learn_durations = map(float, data_learn[:,1])
            curr_learn_amplitudes = map(float, data_learn[:,2])
...
def get_contrasts(subject_id, info): #receives Bunch from subject_info 
    ...
    for i, j in enumerate(info): #iterate through the six run Bunches
        ...
        if 'learn' in j.conditions and 'nonlearn' in j.conditions: #if a subject has both learn and nonlearn trial types
            cont_learn_vs_nonlearn = ['learn_minus_nonlearn', 'T', ['learn','nonlearn'], [1, -1]] #contrast info
            cont_nonlearn_vs_learn = ['nonlearn_minus_learn', 'T', ['learn','nonlearn'], [-1, 1]]
...
def firstlevel_wf(subject_id,sink_directory, name = 'wmaze_frstlvl_wf'):
    ...
    info = dict(task_mri_files = [['subject_id']], #dictionary holding wildcard used in datasource
                motion_noise_files = [['subject_id']])


    #subjectinfo function with name, onset, duration, and amplitude info 
    ...
    #generate a design.mat file for each run
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/Mattfeld_WMAZE/RSA3/'
