...
"""
==============================================
Learning Analysis -- PMA -- Conditional Trials
==============================================
...
- python PMA_fixed_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/learning/PMA/cond
                       -w /scratch/madlab/crash/mandy/learning/PMA/lvl1
...
def subjectinfo(subject_id):
    ...
    data_event = np.genfromtxt(base_proj_dir + '/{0}/model_PMA/set{1}_cond_event.txt'.format(subject_id, curr_set), dtype = str)

    data_pmod = np.genfromtxt(base_proj_dir + '/{0}/model_PMA/set{1}_cond_mod.txt'.format(subject_id, curr_set), dtype = str)
    
    data_all_remaining = np.genfromtxt(base_proj_dir + '/{0}/model_PMA/set{1}_all_remaining_COND.txt'.format(subject_id, curr_set), dtype = str)  
...
def motion_noise(subjinfo, regressor_file):
    ...
    if motion_noise_file.shape[1] > 17: #if there are more than 17 columns in the filter regressors file
        for num_out in range(motion_noise_file.shape[1] - 17): #create a new column name and append it to the names array
            out_name = 'out_{0}'.format(num_out + 1)
            motion_noi_par_names.append(out_name)
    for param_num in range(motion_noise_file.shape[1]): #get the motion noise params in their respective arrays 
...
def firstlevel_wf(subject_id, sink_directory, name = 'wmaze_frstlvl_wf'):
    ...
    #MapNode for function to remove last three volumes from functional data                                
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/PMA/cond/lvl1/3'
