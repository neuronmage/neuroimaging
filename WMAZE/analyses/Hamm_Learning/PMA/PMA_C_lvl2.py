...
"""
===========================================
Model Learning -- PMA -- Conditional Trials
===========================================
...
- python PMA_cond_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/scndlvl/learning/PMA/cond
                       -w /scratch/madlab/crash/mandy/learning/PMA/cond/lvl2
"""
...
def secondlevel_wf(subject_id, sink_directory, name = 'wmaze_scndlvl_wf'):   
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir, 
                          'frstlvl/learning/PMA/cond/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/learning/PMA/cond/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id,curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/learning/PMA/cond/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/learning/PMA/cond/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/learning/PMA/cond/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/PMA/cond/lvl2'
