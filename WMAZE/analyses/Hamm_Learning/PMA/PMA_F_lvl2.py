...
"""
========================================================
Model Learning -- PMA -- Fixed before Conditional Trials
========================================================
Second level (Fixed Effects) workflow for UM GE 750 wmaze task data

- WMAZE Learning analysis Version PMA (parametric modulation analysis)
...
- python PMA_fixed_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/scndlvl/learning/PMA/fixed
                       -w /scratch/madlab/crash/mandy/learning/PMA/fixed/lvl2
"""
...
def get_dofvolumes(dof_files, cope_files):
    ...
    for cope_file_num, cope_file in enumerate(cope_files):
        ...
        cope_name = cope_file_list[-1][7:-7] #name of cope contrast ("A_before_B_corr")
        img = nb.load(cope_file)
        out_data = np.zeros(img.get_shape())
       
        if len(out_data.shape) == 4: #if cope_file is 4 dimensional
            for i in range(out_data.shape[-1]):              
                dof = np.loadtxt(dof_files[cope_file_num][i]) #find appropriate dof file given the current cope               
...
def secondlevel_wf(subject_id, sink_directory, name = 'wmaze_scndlvl_wf'):   
    ...
    all_contrasts = ['AllVsBase', 'pmod', 'event', 'all_remaining']
    contrasts = []
    dof_runs = []

    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir, 
                          'frstlvl/learning/PMA/fixed/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/learning/PMA/fixed/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id,curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/learning/PMA/fixed/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/learning/PMA/fixed/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/learning/PMA/fixed/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/PMA/fixed/lvl2'
