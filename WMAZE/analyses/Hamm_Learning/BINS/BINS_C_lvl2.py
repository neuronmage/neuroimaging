...
- python BINS_cond_lvl2.py -s WMAZE_001
...
#determine the range of copes list
...
def secondlevel_wf(subject_id, sink_directory, name = 'BINS_scndlvl_wf'):    
    ...
    all_contrasts = ['AllVsBase', 'all_remaining', 'cond_bin1', 'cond_bin2', 'cond_bin3',  
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir,
                          'learning/BINS/lvl1/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir,
		             'learning/BINS/lvl1/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'dset/analyses/learning/BINS/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'dset/analyses/learning/BINS/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'sourcedata/mri/preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'dset/analyses/learning/BINS/lvl1/cond/%s/modelfit/dofs/_estimate_model%s/%s')
    ...
    #function node to enerate DOF volume for second level
    ...
    #MapNode node for the fixed effects analysis (aka within subj across runs)
    ...
    #MapNode for FLAMEO to run the fixed effects analysis
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/BINS/cond/lvl2'
