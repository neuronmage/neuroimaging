...
"""
============================================
Model Learning -- BINS -- Conditional Trials
============================================
...
- python RSA1_cond_lvl2.py -s WMAZE_001
                       	   -o /home/data/madlab/Mattfeld_WMAZE/dset/analyses/model_BINS/lvl2/cond
                      	   -w /scratch/data/crash/mandy/learning/BINS/cond/lvl2
"""
...
def get_dofvolumes(dof_files, cope_files):
    ...
    for cope_file_num, cope_file in enumerate(cope_files): #for current cope file in the list of subject copes      
        cope_file_list = cope_file.split('/') #split filename for the current cope file      
        cope_name = cope_file_list[-1][7:-7] #isolate name of the cope contrast ("A_before_B_corr")        
        img = nb.load(cope_file) #use Nibabel to load the nii.gz cope file        
        out_data = np.zeros(img.get_shape()) #Numpy zeros array that is the same dimensions as the cope nii.gz      
        if len(out_data.shape) == 4: #if the cope_file is 4 dimensional            
            for i in range(out_data.shape[-1]): #for each value in the last dimension              
                dof = np.loadtxt(dof_files[cope_file_num][i]) #find the appropriate dof file given the current cope              
                out_data[:, :, :, i] = dof #assignment of dof value to all x, y, z at the current T dimension          
        else: #if cope_file is only 3 dimensional   
...
def secondlevel_wf(subject_id, sink_directory, name = 'BINS_scndlvl_wf'):    
    scndlvl_wf = Workflow(name = 'scndlvl_wf')   
    base_dir = os.path.abspath('/home/data/madlab/Mattfeld_WMAZE/dset/analyses/')
        
    all_contrasts = ['AllVsBase', 'fixed_bin1', 'fixed_bin2', 'fixed_bin3', 'all_remaining',
                     'bin1_minus_bin2', 'bin2_minus_bin1', 'bin1_minus_bin3', 'bin3_minus_bin1', 'bin2_minus_bin3', 'bin3_minus_bin2']
    contrasts = []
    dof_runs = []

    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir, 
                          'learning/BINS/lvl1/fixed/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'learning/BINS/lvl1/fixed/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'dset/analyses/learning/BINS/lvl1/fixed/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'dset/analyses/learning/BINS/lvl1/fixed/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'sourcedata/mri/preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'dset/analyses/learning/BINS/lvl1/fixed/%s/modelfit/dofs/_estimate_model%s/%s')
    ...
    #MapNode to merge all of the copes into a single matrix across subject runs
    ...
    #node to define contrasts from names of copes
...
def create_scndlvl_workflow(args, name = 'BINS_scndlvl'):
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy/learning/BINS/fixed/lvl2'
