...
- python BINS_cond_lvl2.py -s WMAZE_001
                       	   -o /home/data/madlab/Mattfeld_WMAZE/Hamm_Learning/BINS2/lvl2/cond
...
def secondlevel_wf(subject_id, sink_directory, name = 'BINS_scndlvl_wf'):    
    ...
    base_dir = os.path.abspath('/home/data/madlab/Mattfeld_WMAZE/Hamm_Learning/')    
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir,
                          'BINS2/lvl1/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id,curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir,
		             'BINS2/lvl1/cond/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'Hamm_Learning/BINS2/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'Hamm_Learning/BINS2/lvl1/cond/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'sourcedata/mri/preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'Hamm_Learning/BINS2/lvl1/cond/%s/modelfit/dofs/_estimate_model%s/%s')
