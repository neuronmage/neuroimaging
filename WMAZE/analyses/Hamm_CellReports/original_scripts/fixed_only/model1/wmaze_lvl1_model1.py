...
"""
===========================================================
wmaze_fMRI: Mandy Thesis -- Fixed Only -- Model 1 Version 1
===========================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 1 Version 1
   ...
   - Removed last 3 trials before EV creation
     - EV directory (Model 1) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/fixed_model1


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_only/modela
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_only/model1/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        # EVs for *fixed correct* responses in the current run
        data_fixed_corr = np.genfromtxt(base_proj_dir + 
                                        '/{0}/MRthesis/fixed_model1/EVs/run{1}_fixed_corr.txt'.format(subject_id,
                                                                                                      curr_run), 
                                                                                                      dtype = str)
        # EVs for *fixed incorrect* responses in the current run
        data_fixed_incorr = np.genfromtxt(base_proj_dir + 
                                          '/{0}/MRthesis/fixed_model1/EVs/run{1}_fixed_incorr.txt'.format(subject_id,
                                                                                                          curr_run), 
                                                                                                          dtype = str)
        # remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/fixed_model1/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                            curr_run), 
                                                                                                            dtype = str)
        # all remaining responses in the current run
        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/fixed_model1/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)
        
       
        corr_array_name = eval('data_fixed_corr')
        incorr_array_name = eval('data_fixed_incorr')
        if incorr_array_name.size > 0: #MORE THAN ONE MISTAKE MADE
            # Array to contain name of trial:  _corr 
            curr_names = ['fixed_corr', 'fixed_incorr']
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        # If there exists trials in both fixed_corr and fixed_incorr    
        if 'fixed_corr' in j.conditions and 'fixed_incorr' in j.conditions:
            cont_all_corr_vs_all_incorr = ['all_corr_minus_all_incorr', 'T', ['fixed_corr','fixed_incorr'], [1, -1]]
            cont_all_incorr_vs_all_corr = ['all_incorr_minus_all_corr', 'T', ['fixed_corr','fixed_incorr'], [-1, 1]]
...
def firstlevel_wf(subject_id,
    ...
    # List of contrasts with each being a list -[('name', 'stat', [condition list], [weight list], [session list])].
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/fixed_model1/'
