...
- WMAZE Model 7 Version 1.3.2
  ...
  - EV directory (Model 7) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model7


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model7
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model7/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_fixed_before_cond_corr = np.genfromtxt(base_proj_dir + 
                                                    '/{0}/MRthesis/model7/EVs/run{1}_fixed_before_cond_corr.txt'.format(subject_id,
                                                                                                                        curr_run), 
                                                                                                                        dtype = str)
        data_fixed_before_cond_incorr = np.genfromtxt(base_proj_dir + 
                                                      '/{0}/MRthesis/model7/EVs/run{1}_fixed_before_cond_incorr.txt'.format(subject_id,
                                                                                                                            curr_run), 
                                                                                                                            dtype = str) 
        data_same = np.genfromtxt(base_proj_dir + 
                                  '/{0}/MRthesis/model7/EVs/run{1}_fixed_same.txt'.format(subject_id,
                                                                                          curr_run), 
                                                                                          dtype = str)

        data_change = np.genfromtxt(base_proj_dir + 
                                    '/{0}/MRthesis/model7/EVs/run{1}_fixed_change.txt'.format(subject_id,
                                                                                              curr_run), 
                                                                                              dtype = str)

        data_lost = np.genfromtxt(base_proj_dir + 
                                  '/{0}/MRthesis/model7/EVs/run{1}_fixed_lost.txt'.format(subject_id,
                                                                                          curr_run), 
                                                                                          dtype = str)

        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model7/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                  curr_run), 
                                                                                                  dtype = str)

      
        sequence = ['fixed_before_cond']
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        if 'fixed_before_cond_corr' in j.conditions and 'fixed_before_cond_incorr' in j.conditions:
            cont_all_corr_vs_all_incorr = ['corr_minus_incorr', 'T', 
                                           ['fixed_before_cond_corr','fixed_before_cond_incorr'], [1, -1]]
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model7/'
