...
"""
=============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 7 Version 1.3.2
=============================================================================
...
- WMAZE Model 7 Version 1.3.2 
  ...
  - EV directory (Model 3) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model7_2ndTrial


- python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/scndlvl/wmaze_MRthesis/fixed_before_conditional/model7_2ndTrial
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model7_2ndTrial/status/lvl2 
"""
...
def get_dofvolumes(dof_files, cope_files):
    ...
    for cope_file_num, cope_file in enumerate(cope_files):
        ...
        #with open('/home/arenf001/stupid6.txt', 'w') as f:
...
def secondlevel_wf(subject_id,
    ...
    # The contrast names 
    contrasts = ['cond_after_fixed_corr', 'cond_after_fixed_incorr', 'same', 'change', 'lost', 
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/wmaze_MRthesis/fixed_before_conditional/model7-2ndTrial/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
                                                                                                                                                               

    # Should match the number of contrasts (9)
    dof_runs = [[],[],[],[],[],
                [],[],[],[]]
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model7-2ndTrial/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model7-2ndTrial/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model7-2ndTrial/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model7_2ndTrial/lvl2'
