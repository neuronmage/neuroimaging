...
"""
===============================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 7
===============================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 7 Version 1.3.2
  - Use FSL ROI to recreate EPI data, removing last 3 volumes
  - Removed last 3 trials before EV creation
  - EV directory (Model 7) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model7_2ndTrial


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model7_2ndTrial
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model7_2ndTrial/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_cond_after_fixed_corr = np.genfromtxt(base_proj_dir + 
                                                    '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_cond_after_fixed_corr.txt'.format(subject_id,
                                                                                                                                curr_run), 
                                                                                                                                dtype = str)
        data_cond_after_fixed_incorr = np.genfromtxt(base_proj_dir + 
                                                      '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_cond_after_fixed_incorr.txt'.format(subject_id,
                                                                                                                                    curr_run), 
                                                                                                                                    dtype = str) 
        data_same = np.genfromtxt(base_proj_dir + 
                                  '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_fixed_same.txt'.format(subject_id,
                                                                                                   curr_run), 
                                                                                                   dtype = str)

        data_change = np.genfromtxt(base_proj_dir + 
                                    '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_fixed_change.txt'.format(subject_id,
                                                                                                       curr_run), 
                                                                                                       dtype = str)

        data_lost = np.genfromtxt(base_proj_dir + 
                                  '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_fixed_lost.txt'.format(subject_id,
                                                                                                   curr_run), 
                                                                                                   dtype = str)

        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model7_2ndTrial/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                           curr_run), 
                                                                                                           dtype = str)

      
        sequence = ['cond_after_fixed']
        ...
        sequence2 = ['same', 'change', 'lost']
        for curr_type in sequence2:
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        if 'same' in j.conditions and 'change' in j.conditions:
            cont_same_vs_change = ['FFsame_minus_FFchange', 'T', 
                                           ['same','change'], [1, -1]]
            cont_change_vs_same = ['FFchange_minus_FFsame', 'T', 
                                           ['same','change'], [-1, 1]]
            curr_run_contrasts.append(cont_same_vs_change)
            curr_run_contrasts.append(cont_change_vs_same) 
        if 'cond_after_fixed_corr' in j.conditions and 'cond_after_fixed_incorr' in j.conditions:
            cont_all_corr_vs_all_incorr = ['corr_minus_incorr', 'T', 
                                           ['cond_after_fixed_corr','cond_after_fixed_incorr'], [1, -1]]
            cont_all_incorr_vs_all_corr = ['incorr_minus_corr', 'T', 
                                           ['cond_after_fixed_corr','cond_after_fixed_incorr'], [-1, 1]]
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model7_2ndTrial/lvl1/'
