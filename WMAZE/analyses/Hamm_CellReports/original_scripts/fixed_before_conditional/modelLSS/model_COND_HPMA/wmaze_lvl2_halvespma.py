...
"""
========================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model Halves-PMA
========================================================================
Second level (Fixed Effects) workflow for UM GE 750 wmaze task data

- WMAZE Model Halves-PMA 
...
- python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/scndlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model_HALVESPMA/status/lvl2 
"""
...
def get_dofvolumes(dof_files, cope_files):
    ...
    for cope_file_num, cope_file in enumerate(cope_files):
        ...
        # Just the name of the cope contrast
        ...
        # If the cope_file is 4 dimensional
        if len(out_data.shape) == 4:
            ...
            for i in range(out_data.shape[-1]):
                ...
                # Assignment of dof value to all x, y, z at the current T dimension
                out_data[:, :, :, i] = dof  

        # if the cope_file is only 3 dimensional      
        else:
...
def secondlevel_wf(subject_id,
    ...
    for curr_cont in all_contrasts:
        cont_files = glob(os.path.join(base_dir, 
                          'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA/{0}/modelfit/contrasts/_estimate_model*/cope??_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA/%s/modelfit/contrasts/_estimate_model*/cope??_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA/%s/modelfit/contrasts/_estimate_model*/varcope??_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy_crash/model_HALVESPMA/lvl2'
