...
"""
=============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model HALVES_PMA
=============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model HALVESPMA (learning curve - learn v. nonlearn, run1 v. run2, event v. pmod)
  - Use FSL ROI to recreate EPI data, removing last 3 volumes
  - Removed last 3 trials before EV creation
  - EV directory (Model HALVESPMA) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model_HALVESPMA


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVESPMA
...
def subjectinfo(subject_id):
    ...
    data_early_learn_event = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model_COND_HPMA/set{1}_early_learn_event.txt'.format(subject_id, curr_set), dtype = str)

    data_late_learn_event = np.genfromtxt(base_proj_dir + 
                                          '/{0}/MRthesis/model_COND_HPMA/set{1}_late_learn_event.txt'.format(subject_id, curr_set), dtype = str)

    data_early_nonlearn_event = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model_COND_HPMA/set{1}_early_nonlearn_event.txt'.format(subject_id, curr_set), dtype = str)

    data_late_nonlearn_event = np.genfromtxt(base_proj_dir + 
                                             '/{0}/MRthesis/model_COND_HPMA/set{1}_late_nonlearn_event.txt'.format(subject_id, curr_set), dtype = str)

    data_early_learn_pmod = np.genfromtxt(base_proj_dir + 
                                          '/{0}/MRthesis/model_COND_HPMA/set{1}_early_learn_pmod.txt'.format(subject_id, curr_set), dtype = str)

    data_late_learn_pmod = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model_COND_HPMA/set{1}_late_learn_pmod.txt'.format(subject_id, curr_set), dtype = str)

    data_early_nonlearn_pmod = np.genfromtxt(base_proj_dir + 
                                             '/{0}/MRthesis/model_COND_HPMA/set{1}_early_nonlearn_pmod.txt'.format(subject_id, curr_set), dtype = str)

    data_late_nonlearn_pmod = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model_COND_HPMA/set{1}_late_nonlearn_pmod.txt'.format(subject_id, curr_set), dtype = str)
 
    data_all_remaining = np.genfromtxt(base_proj_dir + 
                                       '/{0}/MRthesis/model_COND_HPMA/set{1}_all_remaining.txt'.format(subject_id, curr_set), dtype = str)  
...
def get_contrasts(subject_id, info):
    ...
    if 'early_learn_event' in j.conditions and 'early_learn_pmod' in j.conditions:
        cont_event_early_learn_vs_pmod_early_learn = ['event_early_learn_minus_pmod_early_learn', 'T', 
                                                      ['early_learn_event','early_learn_pmod'], [1, -1]]
        cont_pmod_early_learn_vs_event_early_learn = ['pmod_early_learn_minus_event_early_learn', 'T', 
                                                      ['early_learn_event','early_learn_pmod'], [-1, 1]]
    ...
    if 'late_learn_event' in j.conditions and 'late_learn_pmod' in j.conditions:
        cont_event_late_learn_vs_pmod_late_learn = ['event_late_learn_minus_pmod_late_learn', 'T', 
                                                    ['late_learn_event','late_learn_pmod'], [1, -1]]
        cont_pmod_late_learn_vs_event_late_learn = ['pmod_late_learn_minus_event_late_learn', 'T', 
                                                    ['late_learn_event','late_learn_pmod'], [-1, 1]]
...
def firstlevel_wf(subject_id,
    ...
    datasource.inputs.field_template = dict(task_mri_files = '%s/func/smoothed_fullspectrum/_maskfunc2*/*%s*.nii.gz',
                                            motion_noise_files = '%s/noise/all_filter_regressors.txt')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy_crash/model_HALVESPMA/lvl1/3'
