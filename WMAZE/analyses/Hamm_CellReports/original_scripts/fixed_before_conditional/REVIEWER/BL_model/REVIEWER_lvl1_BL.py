...
- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model3_1-3-2
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model3/model3_1-3-2/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_F_C_corr = np.genfromtxt(proj_dir + '/{0}/MRthesis/model_REVIEW/EVs/run{1}_f_C_corr.txt'.format(subject_id, curr_run), dtype = str)  
        data_F_C_incorr = np.genfromtxt(proj_dir + '/{0}/MRthesis/model_REVIEW/EVs/run{1}_f_C_incorr.txt'.format(subject_id, curr_run), dtype = str) 
        data_F_BL_C = np.genfromtxt(proj_dir + '/{0}/MRthesis/model_REVIEW/EVs/run{1}_f_BL1_C.txt'.format(subject_id, curr_run), dtype = str)   
        data_all_remaining = np.genfromtxt(proj_dir + '/{0}/MRthesis/model_REVIEW/EVs/run{1}_all_remaining.txt'.format(subject_id, curr_run), dtype = str) 
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy_crash/REVIEWER/lvl1/'
