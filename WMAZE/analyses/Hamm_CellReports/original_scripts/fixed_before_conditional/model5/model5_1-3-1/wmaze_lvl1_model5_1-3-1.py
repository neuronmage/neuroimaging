...
"""
=============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 5 Version 1.3.1
=============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 5 Version 1.3.1
...
- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model5_1-3-1/lvl1
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5/model5_1-3-1/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        # Variable to store all EVs for *all before correct B* responses in the current run
        data_all_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model5_1-3-1/EVs/run{1}_all_before_B_corr.txt'.format(subject_id,
                                                                                                               curr_run), 
                                                                                                               dtype = str)

        # Variable to store all EVs for *all before incorrect B* responses in the current run
        data_all_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model5_1-3-1/EVs/run{1}_all_before_B_incorr.txt'.format(subject_id,
                                                                                                                   curr_run), 
                                                                                                                   dtype = str)
        
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5_1-3-1/bleh2_run{0}.txt'.format(curr_run), 'w') as bleh2:
        ...
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model5_1-3-1/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                           curr_run), 
                                                                                                           dtype = str)

        # Variable to store all remaining responses in the current run
        data_last3 = np.genfromtxt(base_proj_dir + 
                                   '/{0}/MRthesis/model5_1-3-1/EVs/run{1}_last3.txt'.format(subject_id,
                                                                                                  curr_run), 
                                                                                                  dtype = str)
        ...
        # INSERT THE ALL REMAINING EV INTO THE MODEL
        ...
        # INSERT THE LAST 3 EV INTO THE MODEL
        names.append(['last_3'])
        onsets.append([map(float, data_last3[:,0])])
        durations.append([map(float, data_last3[:,1])])
        amplitudes.append([map(float, data_last3[:,2])])
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5_1-3-1/bleh_{0}.txt'.format(i+1), 'w') as bleh:
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model5_1-3-1'
