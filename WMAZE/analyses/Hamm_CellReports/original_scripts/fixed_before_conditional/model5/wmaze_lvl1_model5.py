...
"""
=============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 5 Version 1.0.0
=============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 5 Version 1.0.0
 ...
 0 - No method to control or account for the last three volumes/trials
     - EV Directory (Model5)--- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model5


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model5_1-0-0/lvl1
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5/model5_1-0-0/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_all_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model5/EVs/run{1}_all_before_B_corr.txt'.format(subject_id,
                                                                                                           curr_run), 
                                                                                                           dtype = str)
        ...
        data_all_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model5/EVs/run{1}_all_before_B_incorr.txt'.format(subject_id,
                                                                                                               curr_run), 
                                                                                                               dtype = str)
        
        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model5/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                      curr_run), 
                                                                                                      dtype = str)
        # Variable to store all remaining responses in the current run
        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model5/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                  curr_run), 
                                                                                                  dtype = str)
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5/bleh_{0}.txt'.format(i+1), 'w') as bleh:
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model5_1-0-0'
