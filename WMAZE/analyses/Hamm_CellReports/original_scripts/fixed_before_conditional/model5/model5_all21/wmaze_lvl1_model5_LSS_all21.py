...
"""
===============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 5 Version 1.3.1.1
===============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 5 Version 1.3.1.1
 1 - Normal general linear model (not LSS)
 3 - Account for the last three volumes (scanner artifacts)
 1 - Statistically account for last three volumes via statistical modeling
     - Create separate EV for last three volumes -- regressor of no interest
     - EV Directory (Model5 1.3.1)--- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model5_1-3-1
 1 - Target analysis isolating only 10 trials before and 10 trials after largest derivative (greatest increase in accuracy)


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model5_1-3-1-1/lvl1
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5/model5_1-3-1-1/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_set in range(1,4):
        ...
        data_FX_before_COND_corr = np.genfromtxt(base_proj_dir + 
                                                 '/{0}/MRthesis/model5_LSS_targ_all21/EVs/set{1}_all21_FX_before_COND_corr.txt'.format(subject_id,
                                                                                                                                       curr_set), 
                                                                                                                                       dtype = str)
        # Variable to store all EVs for *A before incorrect B* responses in the current run
        data_FX_before_COND_incorr = np.genfromtxt(base_proj_dir + 
                                                   '/{0}/MRthesis/model5_LSS_targ_all21/EVs/set{1}_all21_FX_before_COND_incorr.txt'.format(subject_id,
                                                                                                                                           curr_set), 
                                                                                                                                           dtype = str)
        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model5_LSS_targ_all21/EVs/set{1}_all21_all_remaining.txt'.format(subject_id,
                                                                                                                           curr_set), 
                                                                                                                           dtype = str)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model5_1-3-1-1'
