...
"""
==========================================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Condition - LSS model for beta series correlation
==========================================================================================
A firstlevel workflow for UM GE 750 wmaze task data
This workflow makes use of:
- FSL
For example:
  python wmaze_lvl1_modelLSS_atm12292017.py -s WMAZE_001
                                            -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model5_LSS_targ_split10
                                            -w /scratch/madlab/wmaze/anal_MR_thesis/fixed_before_conditional/model5_LSS_targ_split10/lvl1
...
def subjectinfo(subject_id):
    ...
    # Empty array to contain info from each set (index 1-3)
    output = []

    # For the current set, of which there are 3
    model_counter = 0 #MODEL COUNTER WILL UPDATE FOR EACH TRIAL OF A GIVEN CONDITION
    for curr_set in range(1,4):
        # READ IN THE 3-COLUMN FORMAT TEXT FILES FOR CURRENT RUN EVS
        # Variable to store all EVs for *ALL FIXED (FX) before correct Bs (COND)* responses in the current run
        data_frst10_FX_before_COND_corr = np.genfromtxt(base_proj_dir + 
                                                 '/{0}/MRthesis/model5_LSS_targ_split10/EVs/set{1}_10before_FX_before_COND_corr.txt'.format(subject_id,
                                                                                                                                            curr_set), 
                                                                                                                                            dtype = str)
        
        data_frst10_FX_before_COND_incorr = np.genfromtxt(base_proj_dir + 
                                                   '/{0}/MRthesis/model5_LSS_targ_split10/EVs/set{1}_10before_FX_before_COND_incorr.txt'.format(subject_id,
                                                                                                                                                curr_set), 
                                                                                                                                                dtype = str)
        data_last10_FX_before_COND_corr = np.genfromtxt(base_proj_dir + 
                                                 '/{0}/MRthesis/model5_LSS_targ_split10/EVs/set{1}_10after_FX_before_COND_corr.txt'.format(subject_id,
                                                                                                                                           curr_set), 
                                                                                                                                           dtype = str)
        
        data_last10_FX_before_COND_incorr = np.genfromtxt(base_proj_dir + 
                                                   '/{0}/MRthesis/model5_LSS_targ_split10/EVs/set{1}_10after_FX_before_COND_incorr.txt'.format(subject_id,
                                                                                                                                               curr_set), 
                                                                                                                                               dtype = str)

        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model5_LSS_targ_split10/EVs/set{1}_all_remaining.txt'.format(subject_id,
                                                                                                                       curr_set), 
                                                                                                                       dtype = str)
        ...
        if data_last10_FX_before_COND_incorr.size > 0:
            orig_all_fixed_b4_cond_data = {'frst10_FX_before_COND_corr': data_frst10_FX_before_COND_corr, 
                                           'frst10_FX_before_COND_incorr': data_frst10_FX_before_COND_incorr,
                                           'last10_FX_before_COND_corr': data_last10_FX_before_COND_corr, 
                                           'last10_FX_before_COND_incorr': data_last10_FX_before_COND_incorr}
        else:
            orig_all_fixed_b4_cond_data = {'frst10_FX_before_COND_corr': data_frst10_FX_before_COND_corr,
                                           'last10_FX_before_COND_corr': data_last10_FX_before_COND_corr}
        ...
        for curr_key in orig_all_fixed_b4_cond_data.keys():
            ...
            if curr_condition_data.size == 3: # ONLY ONE EVENT OF THIS CONDITION DURING THIS RUN
                names = [curr_key + '_set{0}_trl{1}'.format(curr_set, trial_counter)]
                ...
                for curr_cond_trl in range(len(curr_condition_data)):
                    ...
                    durations = []
                    amplitudes = []
                    curr_cond_trl_name = curr_key + '_set{0}_trl{1}'.format(curr_set, trial_counter)
                    ...
                    curr_cond_remaining_name = curr_key + '_allbut_set{0}_trl{1}'.format(curr_set, trial_counter)
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        for curr_cond in j.conditions:
            ...
            curr_set_contrasts.append(curr_cont)           
        contrasts.append(curr_set_contrasts)
    return contrasts
...
def get_subs(cons):
    subs = []
    # For each run
    for set_cons in cons:
        set_subs = []
        # For each contrast in the run
        for i, con in enumerate(set_cons):
            # Append a tuple containing "cope#" and "cope#+name" 
            set_subs.append(('cope%d.'%(i + 1), 'cope%02d_%s.'%(i + 1, con[0])))
            set_subs.append(('varcope%d.'%(i + 1), 'varcope%02d_%s.'%(i + 1, con[0])))
            set_subs.append(('zstat%d.'%(i + 1), 'zstat%02d_%s.'%(i + 1, con[0])))
            set_subs.append(('tstat%d.'%(i + 1), 'tstat%02d_%s.'%(i + 1, con[0])))
        subs.append(set_subs)
...
def motion_noise(subjinfo, files):
    ...
    for j,i in enumerate(subjinfo):
        ...
        if 'set1' in i.conditions[0]:
            curr_set = 0
        elif 'set2' in i.conditions[0]:
            curr_set = 1
        elif 'set3' in i.conditions[0]:
            curr_set = 2
        # For each instance of motion_noise_params in the current iteration of subjinfo
        for j3, i3 in enumerate(motion_noise_params[curr_set]):
            i.regressor_names.append(motion_noi_par_names[curr_set][j3])
...
def firstlevel_wf(subject_id,
    ...
    #### expand_epi_files (files_expanded) ----> modelfit_inputspec (functional_data)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model5_LSS_targ_split10'
