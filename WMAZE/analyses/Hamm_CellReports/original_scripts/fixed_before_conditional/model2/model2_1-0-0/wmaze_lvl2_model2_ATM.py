...
================================================================
A secondlevel (or Fixed Effects) workflow for UM GE 750 wmaze task data.
This workflow makes use of:
- FSL
For example::
  python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/lvl2/wmaze_MRthesis/model2/
                       -w /scratch/madlab/wmaze/MRthesis/lvl2/fb4c
    ...
    for cope_file_num, cope_file in enumerate(cope_files):
        ...
        if out_data.shape[-1] != 56:
            ...
            for i in range(out_data.shape[-1]):
                ...
                # Assignment of dof value to the ith dimension
                out_data[:, :, :, i] = dof
        # If the last dimension is 56        
        else:
...
def secondlevel_wf(subject_id,
    ...
    contrasts = ['A_before_B_corr', 'A_before_B_incorr',
                 'C_before_B_corr', 'C_before_B_incorr',
                 'all_A_and_C', 'all_remaining', 
                 'AB_corr_minus_AB_incorr', 'AB_incorr_minus_AB_corr',
                 'CB_corr_minus_CB_incorr', 'CB_incorr_minus_CB_corr',
                 'AC_corr_vs_AC_incorr', 'AC_incorr_vs_AC_corr']
    ...
    #                                           'frstlvl/wmaze_MRthesis/fixed_before_conditional/model2/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    # 12 contrasts                                                                                                                                                           
    #dof_runs = [[],[],[],[],[],[],[],[],[],[],[],[]]
    ...
    '''
    with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model2/meh2.txt', 'w') as meh2:
       for dis in dof_runs:
           meh2.write('{0}\n'.format(dis))
    '''
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model2/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model2/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii')
                                            #dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model2/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model2/lvl2_ATM'
