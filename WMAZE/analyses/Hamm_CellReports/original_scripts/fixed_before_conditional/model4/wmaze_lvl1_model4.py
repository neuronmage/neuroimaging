...
For example:
  python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model4
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model4/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_A_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model4/EVs/run{1}_A_before_B_corr.txt'.format(subject_id,
                                                                                                         curr_run), 
                                                                                                         dtype = str)
        ...
        data_A_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model4/EVs/run{1}_A_before_B_incorr.txt'.format(subject_id,
                                                                                                             curr_run), 
                                                                                                             dtype = str)
        ...
        data_C_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model4/EVs/run{1}_C_before_B_corr.txt'.format(subject_id,
                                                                                                         curr_run), 
                                                                                                         dtype = str)
        ...
        data_C_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model4/EVs/run{1}_C_before_B_incorr.txt'.format(subject_id,
                                                                                                             curr_run), 
                                                                                                             dtype = str)
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model4/bleh2_run{0}.txt'.format(curr_run), 'w') as bleh2:
            bleh2.write('{0} {1} {2} {3}'.format(data_A_before_B_corr, data_A_before_B_incorr, 
                                                 data_C_before_B_corr, data_C_before_B_incorr))
        '''

        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model4/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                      curr_run), 
                                                                                                      dtype = str)
        ...
        for curr_type in sequence:
            ...
            if incorr_array_name.size > 0: #MORE THAN ONE MISTAKE MADE
                ...
                if incorr_array_name.size == 3: #ONLY ONE MISTAKE 
                ...
                # If there was more than one error
                    ...
                    # Array to contain onset time of incorrect trial
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model4/bleh_{0}.txt'.format(i+1), 'w') as bleh:
            bleh.write('{0}  {1}  {2}  {3}'.format('A_before_B_corr' in j.conditions, 'A_before_B_incorr' in j.conditions,
                                                   'C_before_B_corr' in j.conditions, 'C_before_B_incorr' in j.conditions))  
        '''
...
def firstlevel_wf(subject_id,
    ...
    #### motionnoise (subjinfo) ----> specify_model (subject_info)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model4'
