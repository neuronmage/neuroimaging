...
For example:
  python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model0
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model0/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        # Variable to store all EVs for *correct B after A* responses in the current run
        data_corr_B_after_A = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model0/EVs/run{1}_corr_B_after_A.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)

        # Variable to store all EVs for *incorrect B after A* responses in the current run
        data_incorr_B_after_A = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model0/EVs/run{1}_incorr_B_after_A.txt'.format(subject_id,
                                                                                                            curr_run), 
                                                                                                            dtype = str)
         # Variable to store all EVs for *correct B after C* responses in the current run
        data_corr_B_after_C = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model0/EVs/run{1}_corr_B_after_C.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)

        # Variable to store all EVs for *incorrect B after C* responses in the current run
        data_incorr_B_after_C = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model0/EVs/run{1}_incorr_B_after_C.txt'.format(subject_id,
                                                                                                            curr_run), 
                                                                                                            dtype = str)
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model0/bleh2_run{0}.txt'.format(curr_run), 'w') as bleh2:
            bleh2.write('{0} {1} {2} {3}'.format(data_corr_B_after_A, data_incorr_B_after_A, 
                                          data_corr_B_after_A, data_incorr_B_after_C))
        '''

        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model0/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                      curr_run), 
                                                                                                      dtype = str)
        
        ## For all fixed before condition trials ##
        sequence = ['B_after_A', 'B_after_C']
        for curr_type in sequence:
            corr_array_name = eval('data_corr_{0}'.format(curr_type))
            incorr_array_name = eval('data_incorr_{0}'.format(curr_type))    
            # If there are more than one incorrect response in the current run
            if incorr_array_name.size > 0: #MORE THAN ONE MISTAKE MADE
                curr_names = ['corr_{0}'.format(curr_type), 'incorr_{0}'.format(curr_type)]
            ...
            # If there were no errors made at all for the current run
                ...
                curr_names = ['corr_{0}'.format(curr_type)]
                curr_corr_onsets = map(float, corr_array_name[:,0])
                curr_corr_durations = map(float, corr_array_name[:,1])
                curr_corr_amplitudes = map(float, corr_array_name[:,2])
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model0/bleh_{0}.txt'.format(i+1), 'w') as bleh:
            bleh.write('{0}  {1}  {2}  {3}'.format('corr_B_after_A' in j.conditions, 'incorr_B_after_A' in j.conditions,
                                                   'corr_B_after_C' in j.conditions, 'incorr_B_after_C' in j.conditions))  
        '''
        
        # If there exists trials in both B_after_A_corr and B_after_A_incorr    
        if 'corr_B_after_A' in j.conditions and 'incorr_B_after_A' in j.conditions:
            cont_AB_corr_vs_AB_incorr = ['AB_corr_minus_AB_incorr', 'T', ['corr_B_after_A','incorr_B_after_A'], [1, -1]]
            cont_AB_incorr_vs_AB_corr = ['AB_incorr_minus_AB_corr', 'T', ['corr_B_after_A','incorr_B_after_A'], [-1, 1]]
        ...
        if 'corr_B_after_C' in j.conditions and 'incorr_B_after_C' in j.conditions:
            cont_CB_corr_vs_CB_incorr = ['CB_corr_minus_CB_incorr', 'T', ['corr_B_after_C','incorr_B_after_C'], [1, -1]]
            cont_CB_incorr_vs_CB_corr = ['CB_incorr_minus_CB_corr', 'T', ['corr_B_after_C','incorr_B_after_C'], [-1, 1]]
            curr_run_contrasts.append(cont_CB_corr_vs_CB_incorr)
            curr_run_contrasts.append(cont_CB_incorr_vs_CB_corr)
        # If all trial types exist
        if 'corr_B_after_A' in j.conditions and 'corr_B_after_C' in j.conditions and 'incorr_B_after_A' in j.conditions and 'incorr_B_after_C' in j.conditions:
            cont_all_corr_vs_all_incorr = ['all_corr_vs_all_incorr', 'T', 
                                          ['corr_B_after_A','corr_B_after_C', 'incorr_B_after_A', 'incorr_B_after_C'], [1./2, 1./2, -1./2, -1./2]] 
            cont_all_incorr_vs_all_corr = ['all_incorr_vs_all_corr', 'T', 
                                          ['corr_B_after_A','corr_B_after_C', 'incorr_B_after_A', 'incorr_B_after_C'], [-1./2, -1./2, 1./2, 1./2]] 
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model0'
