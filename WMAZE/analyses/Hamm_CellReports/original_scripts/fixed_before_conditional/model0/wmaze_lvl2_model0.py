...
For example::
  python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/lvl2/wmaze_MRthesis/fixed_before_conditional0/
...
def get_dofvolumes(dof_files, cope_files):
    ...
    for cope_file_num, cope_file in enumerate(cope_files):
        ...
        # Just the name of the cope contrast ("corr_B_after_A")
...
def secondlevel_wf(subject_id,
    ...
    # The contrast names
    contrasts = ['corr_B_after_A', 'incorr_B_after_A',
                 'corr_B_after_C', 'incorr_B_after_C',
                 'AB_corr_minus_AB_incorr', 'AB_incorr_minus_AB_corr',
                 'CB_corr_minus_CB_incorr', 'CB_incorr_minus_CB_corr',
                 'all_corr_vs_all_incorr', 'all_incorr_vs_all_corr']
    ...
    for curr_contrast in contrasts:
        cnt_file_list.append(glob(os.path.join(base_dir, 
                                               'frstlvl/wmaze_MRthesis/fixed_before_conditional/model0/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    '''
    with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model0/meh2.txt', 'w') as meh2:
       for dis in dof_runs:
           meh2.write('{0}\n'.format(dis))
    '''
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model0/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model0/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model0/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/lvl2'
