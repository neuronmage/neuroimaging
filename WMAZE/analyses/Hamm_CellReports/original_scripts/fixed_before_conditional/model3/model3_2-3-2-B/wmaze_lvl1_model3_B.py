...
"""
===============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 3 Version 2.3.2-B
===============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model 3 Version 2.3.2-B
...
2  - EV directory (Model 3) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model3_B
B  - Looks at only B trials as a single EV


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model3_B
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model3-B/model3_B/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_all_conditional = np.genfromtxt(base_proj_dir + 
                                             '/{0}/MRthesis/model3_B/EVs/run{1}_all_B.txt'.format(subject_id,
                                                                                                  curr_run), 
                                                                                                  dtype = str)

        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model3_B/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)

        # CONSOLIDATE ALL B TRIAL DATA INTO A DICTIONARY FOR ITERATION
        orig_all_conditional_data = {'all_COND': data_all_conditional}

        # ITERATE OVER THE KEYS OF THE DICTIONARY TO ISOLATE THE CONDITIONS OF INTEREST
        for curr_key in orig_all_conditional_data.keys():
            ...
            copy_all_conditional_data = dict(orig_all_conditional_data)
            curr_condition_data = copy_all_conditional_data.pop(curr_key)
            ...
            if curr_condition_data.size == 3: # ONLY ONE EVENT OF THIS CONDITION DURING THIS RUN
                ...
                for pending_key in copy_all_conditional_data.keys():
                    names.append(pending_key)
                    pending_data = copy_all_conditional_data[pending_key]
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model3_B'
