...
1  - General linear model (not LSS)
3  - Use FSL ROI to recreate EPI data, removing last 3 volumes
2  - Removed last 3 trials before EV creation
     - EV directory (Model 3) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model3


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model3
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model5/model3/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_all_before_B_corr = np.genfromtxt(base_proj_dir + 
                                               '/{0}/MRthesis/model3/EVs/run{1}_all_before_B_corr.txt'.format(subject_id,
                                                                                                              curr_run), 
                                                                                                              dtype = str)
        ...
        data_all_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                                 '/{0}/MRthesis/model3/EVs/run{1}_all_before_B_incorr.txt'.format(subject_id,
                                                                                                                  curr_run), 
                                                                                                                  dtype = str)
        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model3/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                      curr_run), 
                                                                                                       dtype = str)
        # Variable to store all remaining responses in the current run
        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model3/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                  curr_run), 
                                                                                                  dtype = str)
        ...
        ## NONRESPONSE TRIALS ##
        curr_nr_onsets = None
        curr_nr_durations = None
        curr_nr_amplitudes = None
   
        curr_names = ['nonresponse']
        empty = False
        if data_nonresponse.size > 3:
            curr_nr_onsets = map(float, data_nonresponse[:,0])
            curr_nr_durations = map(float, data_nonresponse[:,1])
            curr_nr_amplitudes = map(float, data_nonresponse[:,2])
        elif data_nonresponse.size == 3:
            curr_nr_onsets = [float(data_nonresponse[0])]
            curr_nr_durations = [float(data_nonresponse[1])]
            curr_nr_amplitudes = [float(data_nonresponse[2])]
        else:
            empty = True

        if not empty: # Variables containing all onsets, durations, and amplitudes for nonresponse
            curr_onsets = [curr_nr_onsets]
            curr_durations = [curr_nr_durations]
            curr_amplitudes = [curr_nr_amplitudes] 
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model3/bleh_{0}.txt'.format(i+1), 'w') as bleh:
