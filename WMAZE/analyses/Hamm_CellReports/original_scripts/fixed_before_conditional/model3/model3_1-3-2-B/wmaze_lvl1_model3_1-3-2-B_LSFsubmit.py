...
for i, sid in enumerate(subjs):
    ...
    convertcmd = ' '.join(['python', '/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model3/model3_1-3-2-B/wmaze_lvl1_model3_1-3-2-B.py', '-s', sid, '-o', outdir, '-w', workdir])
    
    # The shell file for each subject
    script_file = 'wmaze_lvl1_model3_1-3-2-B-{0}.sh'.format(sid)
    ...
    outcmd = 'bsub -J atm-wmaze_lvl1_model3_1-3-2-B-{0} -q PQ_madlab -e /scratch/madlab/crash/wmaze_MRthesis/model3_1-3-2-B/lvl1/err_{0} -o /scratch/madlab/crash/wmaze_MRthesis/model3_1-3-2-B/lvl1/out_{0} < {1}'.format(sid, script_file)
