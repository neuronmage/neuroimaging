...
- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model3_1-3-2-B
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model3/model3_1-3-2-B/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        # Variable to store all EVs for *all before correct B* 
        data_corr_B = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model3_1-3-2-B/EVs/run{1}_corr_B.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)
        # Variable to store all EVs for *all before incorrect B*  
        data_incorr_B = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model3_1-3-2-B/EVs/run{1}_incorr_B.txt'.format(subject_id,
                                                                                                            curr_run), 
                                                                                                            dtype = str)
        # Variable to store all remaining 
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model3_1-3-2-B/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                              curr_run), 
                                                                                                              dtype = str)      
        
        
        corr_array_name = eval('data_corr_B')
        incorr_array_name = eval('data_incorr_B')
        if incorr_array_name.size > 0: #MORE THAN ONE MISTAKE MADE

            curr_names = ['corr_B', 'incorr_B']
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        # If there exists trials in both B_corr and B_incorr    
        if 'corr_B' in j.conditions and 'incorr_B' in j.conditions:
            cont_all_corr_vs_incorr = ['all_corr_minus_incorr', 'T', ['corr_B','incorr_B'], [1, -1]]
            cont_all_incorr_vs_corr = ['all_incorr_minus_corr', 'T', ['corr_B','incorr_B'], [-1, 1]]
            curr_run_contrasts.append(cont_all_corr_vs_incorr)
            curr_run_contrasts.append(cont_all_incorr_vs_corr)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model3_1-3-2-B/'
