...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        data_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model3_1-3-2-B/EVs/run{1}_B_corr.txt'.format(subject_id,
                                                                                                        curr_run), 
                                                                                                        dtype = str)
        # Variable to store all EVs for *all before incorrect B*  
        data_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model3_1-3-2-B/EVs/run{1}_B_incorr.txt'.format(subject_id,
                                                                                                            curr_run), 
                                                                                                            dtype = str)
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        if 'B_corr' in j.conditions and 'B_incorr' in j.conditions:
            cont_B_corr_vs_B_incorr = ['B_corr_minus_B_incorr', 'T', ['B_corr','B_incorr'], [1, -1]]
            cont_B_incorr_vs_B_corr = ['B_incorr_minus_B_corr', 'T', ['B_before_B_corr','B_before_B_incorr'], [-1, 1]]
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model3-1-3-2-B/'
