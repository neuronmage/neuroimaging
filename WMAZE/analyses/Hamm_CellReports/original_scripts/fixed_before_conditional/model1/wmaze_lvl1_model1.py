...
"""
================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Condition
================================================================
...
For example:
  python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/model1/
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model1/status/lvl1
...
def subjectinfo(subject_id):
    ...
    for curr_run in range(1,7):
        ...
        # Variable to store all EVs for *A before correct B* responses in the current run
        data_A_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model1/EVs/run{1}_A_before_B_corr.txt'.format(subject_id,
                                                                                                               curr_run), 
                                                                                                               dtype = str)

        # Variable to store all EVs for *A before incorrect B* responses in the current run
        data_A_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model1/EVs/run{1}_A_before_B_incorr.txt'.format(subject_id,
                                                                                                                   curr_run), 
                                                                                                                   dtype = str)
         # Variable to store all EVs for *C before correct B* responses in the current run
        data_C_before_B_corr = np.genfromtxt(base_proj_dir + 
                                            '/{0}/MRthesis/model1/EVs/run{1}_C_before_B_corr.txt'.format(subject_id,
                                                                                                               curr_run), 
                                                                                                               dtype = str)

        # Variable to store all EVs for *C before incorrect B* responses in the current run
        data_C_before_B_incorr = np.genfromtxt(base_proj_dir + 
                                              '/{0}/MRthesis/model1/EVs/run{1}_C_before_B_incorr.txt'.format(subject_id,
                                                                                                                   curr_run), 
                                                                                                                   dtype = str)        
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model1/bleh2_run{0}.txt'.format(curr_run), 'w') as bleh2:
            bleh2.write('{0} {1} {2} {3}'.format(data_A_before_B_corr, data_C_before_B_incorr, 
                                          data_A_before_B_corr, data_C_before_B_incorr))
        '''
        # Variable to store all remaining responses in the current run
        data_all_remaining = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model1/EVs/run{1}_all_remaining.txt'.format(subject_id,
                                                                                                           curr_run), 
                                                                                                           dtype = str)
        # Variable to store all remaining responses in the current run
        data_nonresponse = np.genfromtxt(base_proj_dir + 
                                         '/{0}/MRthesis/model1/EVs/run{1}_nonresponse.txt'.format(subject_id,
                                                                                                         curr_run), 
                                                                                                         dtype = str)

        
        ## For all fixed before condition trials ##
        sequence = ['A_before_B', 'C_before_B']
        for curr_type in sequence:
            ...
            if incorr_array_name.size > 0: #MORE THAN ONE MISTAKE MADE
                ...
                # Variables to contain all onset times, durations, and amplitudes for the current run  
            ...
            # If there were no errors made at all for the current run
            else: #NO MISTAKES WERE MADE
                curr_names = ['{0}_corr'.format(curr_type)]
                curr_corr_onsets = map(float, corr_array_name[:,0])
                curr_corr_durations = map(float, corr_array_name[:,1])
                curr_corr_amplitudes = map(float, corr_array_name[:,2])
                # Variables containing all onsets, durations, and amplitudes for current stim type in the current run
        ...
        ## NONRESPONSE TRIALS ##
        curr_corr_onsets = None
        curr_corr_durations = None
        curr_corr_amplitudes = None
   
        curr_names = ['nonresponse']
        empty = False
        if data_nonresponse.size > 3:
            curr_corr_onsets = map(float, data_nonresponse[:,0])
            curr_corr_durations = map(float, data_nonresponse[:,1])
            curr_corr_amplitudes = map(float, data_nonresponse[:,2])
        elif data_nonresponse.size == 3:
            curr_corr_onsets = [float(data_nonresponse[0])]
            curr_corr_durations = [float(data_nonresponse[1])]
            curr_corr_amplitudes = [float(data_nonresponse[2])]
        else:
            empty = True

        if not empty: # Variables containing all onsets, durations, and amplitudes for current stim type in the current run
            curr_onsets = [curr_corr_onsets]
            curr_durations = [curr_corr_durations]
            curr_amplitudes = [curr_corr_amplitudes] 
...
def get_contrasts(subject_id, info):
    ...
    for i, j in enumerate(info):
        ...
        '''
        with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model1/bleh_{0}.txt'.format(i+1), 'w') as bleh:
            bleh.write('{0}  {1}  {2}  {3}'.format('A_before_B_corr' in j.conditions, 'A_before_B_incorr' in j.conditions,
                                                   'C_before_B_corr' in j.conditions, 'C_before B_incorr' in j.conditions))  
        ''' 
        # If there exists trials in both B_after_A_corr and B_after_A_incorr    
        if 'A_before_B_corr' in j.conditions and 'A_before_B_incorr' in j.conditions:
            cont_AB_corr_vs_AB_incorr = ['AB_corr_minus_AB_incorr', 'T', ['A_before_B_corr','A_before_B_incorr'], [1, -1]]
            cont_AB_incorr_vs_AB_corr = ['AB_incorr_minus_AB_corr', 'T', ['A_before_B_corr','A_before_B_incorr'], [-1, 1]]
            curr_run_contrasts.append(cont_AB_corr_vs_AB_incorr)
            curr_run_contrasts.append(cont_AB_incorr_vs_AB_corr)
            
        # If there exists trials in both B_after_C_corr and B_after_C_incorr    
        if 'C_before_B_corr' in j.conditions and 'C_before_B_incorr' in j.conditions:
            cont_CB_corr_vs_CB_incorr = ['CB_corr_minus_CB_incorr', 'T', ['C_before_B_corr','C_before_B_incorr'], [1, -1]]
            cont_CB_incorr_vs_CB_corr = ['CB_incorr_minus_CB_corr', 'T', ['C_before_B_corr','C_before_B_incorr'], [-1, 1]]
            curr_run_contrasts.append(cont_CB_corr_vs_CB_incorr)
            curr_run_contrasts.append(cont_CB_incorr_vs_CB_corr)
        
        # If all "before" types exist
        if 'A_before_B_corr' in j.conditions and 'C_before_B_corr' in j.conditions and 'A_before_B_incorr' in j.conditions and 'C_before_B_incorr' in j.conditions:
            
            cont_all_AC = ['all_A_and_C', 'T', ['A_before_B_corr','C_before_B_corr', 'A_before_B_incorr', 'C_before_B_incorr'], 
                           [1./4, 1./4, 1./4, 1./4]]
            
            cont_AC_corr_vs_AC_incorr = ['AC_corr_vs_AC_incorr', 'T', 
                                         ['A_before_B_corr','C_before_B_corr', 'A_before_B_incorr', 'C_before_B_incorr'], 
                                         [1./2, 1./2, -1./2, -1./2]] 
            
            cont_AC_incorr_vs_AC_corr = ['AC_incorr_vs_AC_corr', 'T', 
                                         ['A_before_B_corr','C_before_B_corr', 'A_before_B_incorr', 'C_before_B_incorr'], 
                                         [-1./2, -1./2, 1./2, 1./2]] 
            curr_run_contrasts.append(cont_all_AC)
            curr_run_contrasts.append(cont_AC_corr_vs_AC_incorr)
            curr_run_contrasts.append(cont_AC_incorr_vs_AC_corr)
...
'''
# Function for extracting the reaction times from RT files
def reaction_time(subjinfo, files):
    import numpy as np
    reaction_times = []
    reaction_time_name = []
    # If each subject does not have a list of RT or subjinfo files (< 2), make it a list of one
    if not isinstance(files, list):
        files = [files]
    if not isinstance(subjinfo, list):
        subjinfo = [subjinfo]
    # For each reaction time file (6 in total)
    for curr_RT_file in files:
        # Create name for each column (only one)
        curr_reaction_time_name = 'RT'
        # Create numpy array of each reaction time file
        a = np.genfromtxt(curr_RT_file)
        a2 = np.zeros((200,))
        a2[:a.shape[0]] = a
        a = a2
        reaction_times.append(a)
        reaction_time_name.append(curr_reaction_time_name)
          
    # For each instance of subjinfo files (6 in total)    
    for i, curr_subjinfo in enumerate(subjinfo):
        curr_subjinfo.regressor_names.append(reaction_time_name[i])
        curr_subjinfo.regressors.append(reaction_times[i])
...
def firstlevel_wf(subject_id,
    ...
    info = dict(task_mri_files = [['subject_id', 'wmaze']],
                ...
                reaction_time_files = [['subject_id']])
    ...
    #### subject_info(output) ----> getsubs (info)
    #### getcontrasts (contrasts) ----> getsubs (cons)
    ...
    datasource.inputs.field_template = dict(task_mri_files = 'preproc/%s/func/smoothed_fullspectrum/_maskfunc2*/*%s*.nii.gz',
                                            # Filter regressor noise files
                                            motion_noise_files = 'preproc/%s/noise/%s*.txt',
                                            reaction_time_files = 'scanner_behav/%s/MRthesis/RTs/run*_RTs.txt')
    ...
    #### datasource (motion_noise_files) ----> motionnoise (files)

    '''
    # Function node to modify the reaction time to be a regressor of no interest
    reactiontime = Node(Function(input_names = ['subjinfo', 'files'],
                                output_names = ['subjinfo'],
                                # Calls the function 'reaction_time'
                                function = reaction_time),
                       name = 'reactiontime')
    reactiontime.inputs.ignore_exception = False
    frstlvl_wf.connect(motionnoise, 'subjinfo', reactiontime, 'subjinfo')
    frstlvl_wf.connect(datasource, 'reaction_time_files', reactiontime, 'files')
    '''

    #### motionnoise (subjinfo) ----> reactiontime (subjinfo)
    #### datasource (task_mri_files) ----> reactiontime (files)
    ...
    # Data files for model -- list of 4D files or nested list of 3D files per session
    ...
    #### datasource (task_mri_files) ----> specify_model (functional_runs)
    #### reactiontime (subjinfo) ----> specify_model (subject_info)
    ...
    #### datasource (task_mri_files) ----> modelfit_inputspec (functional_data)
    #### getcontrasts (contrasts) ----> modelfit (constrasts)
    #### specify_model (session_info) ----> modelfit_inputspec (session_info)
    ...
    #### modelfit_inputspec (interscan_interval) ----> level1_design (interscan_interval)
    #### modelfit_inputspec (session_info) ----> level1_design (session_info)
    #### modelfit_inputspec (contrasts) ----> level1_design (constrasts)
    #### modelfit_inputspec (bases) ----> level1_design (bases)
    #### modelfit_inputspec (model_serial_correlations) ----> level1_design (model_serial_correlations)
    ...
    #### level1_design (fsf_files) ----> generate_model (fsf_files)
    #### level1_design (ev_files) ----> generate_model (ev_files)
    ...
    #### modelfit_inputspec (film_threshold) ----> estimate_model (threshold)
    #### modelfit_inputspec (functional_data) ----> estimate_model (in_file)
    ...
    #### estimate_model (zstats) ----> merge_contrasts (in1)
    ...
    #### merge_contrasts (out) ----> z2pval (in_file)
    ...
    #### estimate_model (copes) ----> modelfit_outputspec (copes)
    #### estimate_model (varcopes) ----> modelfit_outputspec (varcopes)
    #### merge_contrasts (out) ----> modelfit_outputspec (zstats)
    #### z2pval (out_file) ----> modelfit_outputspec (pfiles)
    #### generate_model (design_image) ----> modelfit_outputspec (design_image)
    #### generate_model (design_file) ----> modelfit_outputspec (design_file)
    #### generate_model (design_cov) ----> modelfit_outputspec (design_cov)
    #### generate_model (param_estimates) ----> modelfit_outputspec (parameter_estimates)
    #### generate_model (dof_file) ----> modelfit_outputspec (dof_file)
    #### generate_model (sigmasquareds) ----> modelfit_outputspec (sigmasquareds)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis'
