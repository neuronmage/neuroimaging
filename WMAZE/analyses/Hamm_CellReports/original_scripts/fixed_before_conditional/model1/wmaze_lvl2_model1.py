...
For example::
  ...
  python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/lvl2/wmaze_MRthesis/fixed_before_conditional/model1
...
def secondlevel_wf(subject_id,
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                                               'frstlvl/wmaze_MRthesis/fixed_before_conditional/model1/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
                                                                                                                                                               
    # Should match the number of contrasts (13)
    dof_runs = [[],[],[],[],[],[],[],[],[],[],[]]
    ...
    '''
    with open('/home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model1/meh2.txt', 'w') as meh2:
       for dis in dof_runs:
           meh2.write('{0}\n'.format(dis))
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model1/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model1/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model1/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model1/lvl2'
