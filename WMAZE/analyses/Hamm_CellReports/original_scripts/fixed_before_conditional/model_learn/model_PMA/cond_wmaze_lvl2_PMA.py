...
def secondlevel_wf(subject_id,
    ...
    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir, 
                          'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_PMA/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_PMA/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_PMA/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_PMA/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_PMA/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model_cond_PMA/lvl2'
