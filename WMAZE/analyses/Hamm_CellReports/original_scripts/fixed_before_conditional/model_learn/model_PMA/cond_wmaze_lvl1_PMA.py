...
def subjectinfo(subject_id):
    ...
    data_PMA_pmod = np.genfromtxt(base_proj_dir + 
                                       '/{0}/MRthesis/model_PMA/EVs/set{1}_cond_mod.txt'.format(subject_id, curr_set), dtype = str)

    data_PMA_event = np.genfromtxt(base_proj_dir + 
                                       '/{0}/MRthesis/model_PMA/EVs/set{1}_cond_event.txt'.format(subject_id, curr_set), dtype = str)
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model_cond_PMA/lvl1/3'
