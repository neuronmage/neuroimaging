...
"""
=============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model Halves
=============================================================================
First level workflow for UM GE 750 wmaze task data

- WMAZE Model Learn Version 3 (learning curve - learn v. nonlearn, compared between runs)
  - Use FSL ROI to recreate EPI data, removing last 3 volumes
  - Removed last 3 trials before EV creation
  - EV directory (Model Halves) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model_HALVES


- python wmaze_lvl1.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/frstlvl/wmaze_MRthesis/fixed_before_conditional/model_HALVES
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model_HALVES/status/lvl1
...
def subjectinfo(subject_id):
    ...
    data_early_learning = np.genfromtxt(base_proj_dir + 
                                        '/{0}/MRthesis/model_HALVES/set{1}_early_learn.txt'.format(subject_id, curr_set), dtype = str)

    data_late_learning = np.genfromtxt(base_proj_dir + 
                                       '/{0}/MRthesis/model_HALVES/set{1}_late_learn.txt'.format(subject_id, curr_set), dtype = str)

    data_early_nonlearning = np.genfromtxt(base_proj_dir + 
                                           '/{0}/MRthesis/model_HALVES/set{1}_early_nonlearn.txt'.format(subject_id, curr_set), dtype = str)

    data_late_nonlearning = np.genfromtxt(base_proj_dir + 
                                          '/{0}/MRthesis/model_HALVES/set{1}_late_nonlearn.txt'.format(subject_id, curr_set), dtype = str)
 
    data_all_remaining = np.genfromtxt(base_proj_dir + 
                                       '/{0}/MRthesis/model_HALVES/set{1}_all_remaining.txt'.format(subject_id, curr_set), dtype = str)  
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/mandy_crash/model_HALVES/lvl1/3'
