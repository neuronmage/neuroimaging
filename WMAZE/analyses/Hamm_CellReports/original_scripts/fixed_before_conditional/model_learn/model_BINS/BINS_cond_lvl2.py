...
"""
==============================================================================
wmaze_fMRI: Mandy Thesis -- Fixed before Conditional -- Model 3 Version Curve
==============================================================================
Second level (Fixed Effects) workflow for UM GE 750 wmaze task data

- WMAZE Model 3 Version Learn1 
  - Use FSL ROI to recreate EPI data, removing last 3 volumes
  - Removed last 3 trials before EV creation
  - EV directory (Model 3) --- /home/data/madlab/data/mri/wmaze/scanner_behav/WMAZE_001/MRthesis/model_curve


- python wmaze_lvl2.py -s WMAZE_001
                       -o /home/data/madlab/data/mri/wmaze/scndlvl/wmaze_MRthesis/fixed_before_conditional/model_curve
                       -w /home/data/madlab/scripts/wmaze/anal_MR_thesis/fixed_before_conditional/model_curve/status/lvl2 
"""
...
def secondlevel_wf(subject_id,
    ...
    all_contrasts = ['AllVsBase', 'all_remaining', 
                     'curve_bin1', 'curve_bin2', 'curve_bin3',  
                     'bin1_minus_bin2', 'bin2_minus_bin1', 
                     'bin1_minus_bin3', 'bin3_minus_bin1',
                     'bin2_minus_bin3', 'bin3_minus_bin2']
    contrasts = []
    dof_runs = []

    for i, curr_cont in enumerate(all_contrasts):
        cont_files = glob(os.path.join(base_dir, 
                          'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_curve/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_cont)))
        if len(cont_files) > 1:
    ...
    for curr_contrast in contrasts:          
        cnt_file_list.append(glob(os.path.join(base_dir, 
                             'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_curve/{0}/modelfit/contrasts/_estimate_model*/cope*_{1}.nii.gz'.format(subject_id, curr_contrast)))) 
    ...
    datasource.inputs.field_template = dict(copes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_curve/%s/modelfit/contrasts/_estimate_model*/cope*_%s.nii.gz',
                                            varcopes = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_curve/%s/modelfit/contrasts/_estimate_model*/varcope*_%s.nii.gz',
                                            mask_file = 'preproc/%s/ref/_fs_threshold20/%s*_thresh.nii',
                                            dof_files = 'frstlvl/wmaze_MRthesis/fixed_before_conditional/model_cond_curve/%s/modelfit/dofs/_estimate_model%s/%s')
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/crash/wmaze_MRthesis/model_cond_curve/lvl2'
