...
def convert_dicoms(subjs, dicom_dir_template, outdir, heuristic_file, converter,
    ...
    for sid in subjs:
        if queue:
            ...
            outcmd = 'bsub -J atm-%s_%s -q %s -e /scratch/madlab/crash/dcm_err_%s_%s -o /scratch/madlab/crash/dcm_out_%s_%s < %s' % (sid, session, queue, sid, session, sid, session, script_file)
            os.system(outcmd)
            continue
