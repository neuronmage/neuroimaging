...
# Compute motion regressors up to given order and derivative motion + d(motion)/dt + d2(motion)/dt2 (linear + quadratic)
...
# Builds a regressor set comprising motion parameters, composite norm, and outliers
...
"""
def extract_noise_components(realigned_file, mask_file, num_components = 5, extra_regressors = None):
...
def create_workflow(func_runs,
    ...
    # Run AFNI's despike. This is always run, however, whether this is fed to realign depends on the input configuration
    ...
    # Calculate the transformation matrix from EPI space to FreeSurfer space using the BBRegister command
    ...
    # Create a filter text file to remove motion (+ derivatives), art confounds, as well as 1st, 2nd, and 3rd order legendre polynomials
    ...
    # Obtain the temporal mean of maskfunc
    ...
    # Merge the smooth median and smooth temporal mean
...
def create_preproc_workflow(args, 
                            name = 'wmaze'):
    import numpy as np
    TR = args.tr
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/wmaze/crash'
