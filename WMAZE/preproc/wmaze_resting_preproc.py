...
For example::
  ...
  python wmaze_resting_preproc.py -r 1 2 -s WMAZE_001
...
def create_workflow(func_runs,
    ...
    if slice_times is not None:
        ...
        motion_correct.plugin_args = {'bsub_args': '-n %s' %os.environ['OMP_NUM_THREADS']}
    ...
    wf.connect(fs_threshold2, ('binary_file', pickfirst), outputnode, 'mask_file')
    ...
    wf.connect(motion_correct, ('out_file', rename, '_filtermotart'),
    ...
    wf.connect(outputnode, 'realigned_files', datasink, 'wmaze_rest.func.realigned')
    wf.connect(outputnode, 'motion_plots', datasink, 'wmaze_rest.motion.@plots')
    wf.connect(outputnode, 'mask_file', datasink, 'wmaze_rest.ref.@mask')
    wf.connect(outputnode, 'smoothed_files', datasink, 'wmaze_rest.func.smoothed_bandpassed')
    ...
    wf.connect(outputnode, 'reg_fsl_file', datasink, 'wmaze_rest.bbreg.@regfsl')
    wf.connect(outputnode, 'artnorm_files', datasink, 'wmaze_rest.art.@norm_files')
    wf.connect(outputnode, 'artoutlier_files', datasink, 'wmaze_rest.art.@outlier_files')
    wf.connect(outputnode, 'artdisplacement_files', datasink, 'wmaze_rest.art.@displacement_files')
    wf.connect(outputnode, 'motion_parameters_plusDerivs', datasink, 'wmaze_rest.noise.@motionplusDerivs')
    wf.connect(outputnode, 'motionandoutlier_noise_file', datasink, 'wmaze_rest.noise.@motionplusoutliers')
...
def create_preproc_workflow(args, name='wmaze_rest'):
    import numpy as np
    TR = args.tr
...
if __name__ == "__main__":
    ...
    wf.config['execution']['crashdump_dir'] = '/scratch/madlab/wmaze_rest/crash'
