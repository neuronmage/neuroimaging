...
def process_dicoms(fl):
    ...
    for series, mwidx in sorted(group_map.items()):
        ...
        try:
            ...
            TE = -1
        info = [total, os.path.split(files[0])[1], series, '-', '-', '-'] + size + [TR, TE, dcminfo.SeriesDescription]
        seqinfo.append(info)
    ...
    for sid in subjs:
        if queue:
            ...
            convertcmd = ' '.join(['python', progname, '-d', dicom_dir_template,
                                   ...
                                   '-c', converter, '-wd', disc])
            script_file = 'atm-%s_%s.sh' %(sid, disc)
            with open(script_file, 'wt') as fp:
                fp.writelines(['#!/bin/bash\n', convertcmd])
            outcmd = 'sbatch -J atm-%s -p investor \
            --qos %s -e /scratch/madlab/crash/dcm_err_%s_%s \
            -o /scratch/madlab/crash/dcm_out_%s_%s < %s' % (sid, queue, sid, disc, sid, disc, script_file)
            os.system(outcmd)
            continue
        if disc == 'multiple':
            if os.path.isdir(dicom_dir_template + sid + '/disc1' + '/B'):
                disc1_sdir1 = dicom_dir_template + sid + '/disc1' + '/A/*'
                disc1_sdir2 = dicom_dir_template + sid + '/disc1' + '/B/*'
                disc1_fl1 = sorted(glob(disc1_sdir1))
                disc1_fl2 = sorted(glob(disc1_sdir2))
                disc1_fl = sorted(disc1_fl1 + disc1_fl2)
            else:
                disc1_sdir = dicom_dir_template + sid + '/disc1' + '/A/*'
                disc1_fl = sorted(glob(disc1_sdir))
            
            if os.path.isdir(dicom_dir_template + sid + '/disc2' + '/B'):
                disc2_sdir1 = dicom_dir_template + sid + '/disc2' + '/A/*'
                disc2_sdir2 = dicom_dir_template + sid + '/disc2' + '/B/*'
                disc2_fl1 = sorted(glob(disc2_sdir1))
                disc2_fl2 = sorted(glob(disc2_sdir2))
                disc2_fl = sorted(disc2_fl1 + disc2_fl2)
            else:
                disc2_sdir = dicom_dir_template + sid + '/disc2' + '/A/*'
                disc2_fl = sorted(glob(disc2_sdir))
            
            #if os.path.isdir(dicom_dir_template + '/' + sid + '/disc3' + '/B'):
            #    disc3_sdir1 = dicom_dir_template + '/' + sid + '/disc3' + '/A/*'
            #    disc3_sdir2 = dicom_dir_template + '/' + sid + '/disc3' + '/B/*'
            #    disc3_fl1 = sorted(glob(disc3_sdir1))
            #    disc3_fl2 = sorted(glob(disc3_sdir2))
            #    disc3_fl = sorted(disc3_fl1 + disc3_fl2)
            #else:
            #    disc3_sdir = dicom_dir_template + '/' + sid + '/disc3' + '/A/*'
            #    disc3_fl = sorted(glob(disc3_sdir))
            #fl = sorted(disc1_fl + disc2_fl + disc3_fl)
            fl = sorted(disc1_fl + disc2_fl)
        elif disc == 'single':
...
"""))
    ...
    parser.add_argument('-wd','--whichdisc', dest='whichdisc',
                        help='how many discs did the dicoms come on - mutliple or single')
