...
filename = _thisDir + os.sep + 'wmazeblprob_data/%s_%s_%s' %(expInfo['participant'], expName, expInfo['date'])
...
#Our own log file
datFile=open('wmazeblprob_data' + os.sep + '%s_wmazebl_%s.txt' %(expInfo['participant'], expInfo['date']),'a')
datFile.write('Run\tTrial\tStim\tStimOnset\tTrialType\tResp\tFBOnset\tValidFB\tCorrect\tIncorrect\tRT\n')
...
def BaselineTrial(bl_dict):
    ...
    #blbkgrnd_image = (np.random.rand(256,160) > 0.85) * 1

    target_change = 0.85 - bl_dict['talpha']
    foil_change = 0.85 - bl_dict['falpha']
    
    bltarget_image = (np.random.rand(320,200) > target_change) * 1
    blfoil_image = (np.random.rand(320,200) > foil_change) * 1
...
all_kaleidos = all_kaleidos[385:-1]
...
for thisRun in runs:
    ...
    for thisTrial in trials:
        ...
        #-------Start Routine "trial"-------
        continueRoutine = True
        give_valid_fb = np.random.rand(1)
            ...
            if trial_type == 3:
                ...
                if key_resp.status == STARTED:
                    ...
                    if len(theseKeys) > 0:  # at least one key was pressed
                        ...
                        if key_resp.keys == '1' and trial_type != 1:
                            feedback_box.pos = [-0.5, 0]
                            if correct_resp == '1' and give_valid_fb > 0.2:
                                valid_fb = 1
                                feedback_text.text = "Yes!"
                                feedback_text.color = u'green'
                                correct, incorrect = [1, 0]
                            elif correct_resp == '1' and give_valid_fb < 0.2:
                                valid_fb = 0
                                feedback_text.text = "No!"
                                feedback_text.color = u'red'
                                correct, incorrect = [1, 0]
                            elif correct_resp == '2' and give_valid_fb > 0.2:
                                valid_fb = 1
                                feedback_text.text = "No!"
                                feedback_text.color = u'red'
                                correct, incorrect = [0, 1]
                                ...
                                feedback_text.color = u'green'
                                correct, incorrect = [0, 1]
                        elif key_resp.keys == '2' and trial_type != 1:
                            feedback_box.pos = [0.5, 0]
                            if correct_resp == '2' and give_valid_fb > 0.2:
                                valid_fb = 1
                                feedback_text.text = "Yes!"
                                feedback_text.color = u'green'
                                correct, incorrect = [1, 0]
                                ...
                                feedback_text.color = u'green'
                                correct, incorrect = [0, 1]
                        elif key_resp.keys == '1' and trial_type == 1:
                            feedback_box.pos = [-0.5, 0]
                            if correct_resp == '1':
                                valid_fb = 1
                                feedback_text.text = "Yes!"
        ...
        datFile.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n'%(run,trials.thisTrialN+1,trialTOnset,Trial_Type,key_resp.keys,FBTOnset,valid_fb,correct,incorrect,key_resp.rt))
